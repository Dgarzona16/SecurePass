...
import com.dgarzona16.securepass.ui.layout.activity.ActivityViewModel
    ...
    LaunchedEffect(viewModel.configsStatus.value){
        viewModel.apply {
            if (configsStatus.value == GetConfigStatus.SUCCESS){
                val theme = config.first { it.id == PREF_THEME }
                themeState.setThemeSelection(ThemeSelection.valueOf(theme.value))
            }
        }
    }
